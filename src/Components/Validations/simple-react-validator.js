...
var SimpleReactValidator = function () {
  function SimpleReactValidator() {
    ...
    this.rules = _extends({
      ...
      url: { message: 'The :attribute must be a url.', rule: function rule(val) {
		...
		required_selected: { message: 'The :attribute field is required.', rule: function rule(val) {
			if(val==''||val==0){
				return false	
			}else if(val=='undefined'){
				return false
			}else if(val!='undefined' && val.id==0){
				return false
			}else{
				return true
			}
        } },
        required_boolean: { message: 'The :attribute field is required.', rule: function rule(val) {
  ...
  _createClass(SimpleReactValidator, [{
    ...
    value: function _reactErrorElement(message, customClass) {
      return React.createElement('span', { className: customClass || 'validation-message' }, message);
    }
  }, {
    key: '_reactSuccessElement',
    value: function _reactErrorElement(message, customClass) {
      return React.createElement('div', { className: customClass || 'validation-success-message' }, message);
    }
  }, {
    key: '_testRegex',
